    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
      ...
      try {
        ...
        Statement statement = Dbcon.connect();
            ResultSet resultSet = statement.executeQuery("SELECT * FROM user where dmail='" + username1 + "' AND 	dpass='" + password + "';");
            System.out.println("SELECT * FROM user where dmail='" + username1 + "' AND 	dpass='" + password + "'");
            if (resultSet.next()) {
                 
                session.setAttribute("did",resultSet.getString("did"));
//                session.setAttribute("email", email);
                session.setAttribute("UserType", "doctor");
                session.setAttribute("flash_message", "Logged in successfully !!!");
                session.setAttribute("flash_type", "success");
               response.sendRedirect("detection_byimage.jsp");
